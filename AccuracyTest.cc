...
//Global Variables. Which are the constant for the kobuki packet
unsigned char header0 = 0xaa;
unsigned char header1 = 0x55;
static double distanceTraveled, heading;
...
void updateLocation(unsigned short leftEncoder, unsigned short rightEncoder) {
	...
	cout << "\nPose update: " << "x[" << xPosition*1000 << "] y[" << yPosition*1000 << "] heading[" << theta << "]" << endl;
	
	oldX = xPosition;
	oldY = yPosition; 
	
	distanceTraveled += wheelRadius*((tickToRad*leftDiffTicks) + (tickToRad*rightDiffTicks))/2.0;
	heading += wheelRadius*((tickToRad*rightDiffTicks) - (tickToRad*leftDiffTicks))/bias;	
}
...
void setLed(int Led) {
		...
		unsigned int length = 6;	// TODO Find length of led is not 6 and send packet for the led
}

void setSound(int Sound) {		//TODO send packet for sound
		KobukiCommand CommandKobuki;
		unsigned int length = 6;
		unsigned int commandData = 1;	
}
...
int main() {
	...
	********************************/
		...
		/**if (distanceTraveled >= 1.0 && heading >= 1.570796327) {
			distanceTraveled = 0;
			heading = 0;
			setBaseControl(0,0);
		}
		else if (distanceTraveled >= 1.0) 
			setBaseControl(150,1);
		else
			setBaseControl(150,0); */
			
	/********************************
	********************************/
	
	setBaseControl(300, 350);
